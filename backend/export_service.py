...
def generate_css(theme: Dict[str, Any]) -> str:
    ...
    * {{ margin:0; padding:0; box-sizing:border-box; }}
    ...
    .btn-outline:hover {{ background:#fff; color:{sc}; }}
    /* Sections */
    ...
    .gallery-grid.masonry {{ grid-template-columns:repeat(3,1fr); grid-auto-rows:200px; }}
    ...
    .syroce-brand {{ display:flex; align-items:center; gap:6px; font-size:0.85rem; opacity:0.7; }}
...
def render_header(props: Dict[str, Any], theme: Dict[str, Any]) -> str:
...
def render_hero(props: Dict[str, Any], theme: Dict[str, Any]) -> str:
    ...
    layout = props.get("layout", "fullscreen")
    cta_html = f'<a href="{cta_link}" class="btn">{cta_text}</a>' if cta_text else ""
    return f"""
    <section class="hero {layout}" id="anasayfa">
        <div class="hero-bg" style="background-image:url('{bg}')"></div>
        <div class="hero-overlay" style="opacity:{opacity}"></div>
...
def render_about(props: Dict[str, Any], theme: Dict[str, Any]) -> str:
    title = props.get("title", "")
    desc = props.get("description", "")
    image = props.get("image", "")
    layout = props.get("layout", "left-image")
    img_first = layout != "right-image"
        ...
        <section class="section" id="hakkimizda">
            <div class="container">
                <div class="about-grid">
                    <div>{img_html}</div>
                    <div class="about-text"><h2>{title}</h2><p>{desc}</p></div>
                </div>
            </div>
        </section>
        """
...
def render_rooms(props: Dict[str, Any], theme: Dict[str, Any]) -> str:
    title = props.get("title", "Odalarimiz")
    subtitle = props.get("subtitle", "")
    ...
    for room in rooms:
        ...
        rooms_html += f"""
        <div class="room-card">
            <img src="{room.get('image', '')}" alt="{room.get('name', '')}">
            <div class="room-card-body">
                <h3>{room.get('name', '')}</h3>
                <p>{room.get('description', '')}</p>
                <div class="room-price">{room.get('price', '')}</div>
                <div class="room-features">{features_html}</div>
            </div>
        </div>
        """
    return f"""
    <section class="section" id="odalar">
        <div class="container">
            ...
            <div class="rooms-grid">{rooms_html}</div>
        </div>
    </section>
    """


def render_gallery(props: Dict[str, Any], theme: Dict[str, Any]) -> str:
    title = props.get("title", "Galeri")
    ...
    return f"""
    <section class="section" style="background:#f8f8f8" id="galeri">
        <div class="container">
            <h2 class="section-title">{title}</h2>
            <div class="gallery-grid {grid_class}">{images_html}</div>
        </div>
    </section>
    """


def render_services(props: Dict[str, Any], theme: Dict[str, Any]) -> str:
    title = props.get("title", "Hizmetlerimiz")
...
def render_testimonials(props: Dict[str, Any], theme: Dict[str, Any]) -> str:
    title = props.get("title", "Misafir Yorumlari")
    ...
    for t in testimonials:
        ...
        <div class="testimonial-card">
            ...
            <p>\"{t.get('text', '')}\"</p>
            <div class="testimonial-author">{t.get('name', '')}</div>
        </div>
        """
    return f"""
    <section class="section" style="background:#f8f8f8" id="yorumlar">
        <div class="container">
            <h2 class="section-title">{title}</h2>
            <div class="testimonials-grid">{items_html}</div>
        </div>
    </section>
    """


def render_contact(props: Dict[str, Any], theme: Dict[str, Any]) -> str:
    title = props.get("title", "Iletisim")
            ...
            <div class="contact-grid">
                <div class="contact-info">
                    ...
                    <div class="contact-item">
                        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="{pc}" stroke-width="2"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg>
                        <span>{address}</span>
                    </div>
                    <div class="contact-item">
                        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="{pc}" stroke-width="2"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 22 16.92z"/></svg>
                        <span>{phone}</span>
                    </div>
                    <div class="contact-item">
                        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="{pc}" stroke-width="2"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>
                        <span>{email}</span>
                    </div>
                </div>
...
def render_banner(props: Dict[str, Any], theme: Dict[str, Any]) -> str:
    title = props.get("title", "")
    subtitle = props.get("subtitle", "")
    bg = props.get("backgroundImage", "")
    cta_text = props.get("ctaText", "")
...
def render_footer(props: Dict[str, Any], theme: Dict[str, Any]) -> str:
    hotel_name = props.get("hotelName", "Hotel")
    address = props.get("address", "")
    phone = props.get("phone", "")
    email = props.get("email", "")
    ...
    for platform, link in social.items():
        social_html += f'<a href="{link}" title="{platform.capitalize()}">{platform[0].upper()}</a>'
    return f"""
    <footer class="site-footer">
        <div class="container">
            <div class="footer-grid">
                ...
                <div>
                    <h3>Hizli Erisim</h3>
                    <p><a href="#anasayfa">Anasayfa</a></p>
                    <p><a href="#odalar">Odalar</a></p>
                    <p><a href="#galeri">Galeri</a></p>
                    <p><a href="#iletisim">Iletisim</a></p>
                </div>
                <div>
                    <h3>Iletisim</h3>
                    <p>{phone}</p>
            ...
            <div class="footer-bottom">
                <p>&copy; 2025 {hotel_name}. Tum haklari saklidir.</p>
                <a href="https://syroce.com" class="syroce-brand" target="_blank">Powered by <span>Syroce</span></a>
            </div>
        </div>
    </footer>
    """
...
def generate_full_html(project_data: Dict[str, Any]) -> str:
    ...
    fonts_url = "https://fonts.googleapis.com/css2?" + "&".join([f"family={f.replace(' ', '+')}:wght@400;500;600;700" for f in font_families]) + "&display=swap"
    ...
    for section in sections:
        if section.get("visible", True):
            ...
            if renderer:
                sections_html += renderer(section.get("props", {}), theme)
...
## Kurulum
1. Bu klasoru web sunucunuza yukleyin
2. index.html dosyasini tarayicinizda acin
3. Hosting'e yonlendirmek icin DNS ayarlarinizi yapin

## Hosting Notlari
{project_data.get('hosting_notes', 'Henuz hosting bilgisi eklenmedi.')}

## Domain Bilgileri
{project_data.get('domain_notes', 'Henuz domain bilgisi eklenmedi.')}
